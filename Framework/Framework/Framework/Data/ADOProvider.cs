...
namespace Framework.Data
{
    ...
    public class ADOProvider : IDisposable
    {
        ...
        public virtual T QueryById<T>(int id)
        {
            ...
            if (propertyEntity != null && propertyTableEntity != null)
            {
                ...
                using (IDbConnection cn = Connection)
                {
                    ...
                    item =
                        cn.Query<T>(
                            string.Format("SELECT * FROM [{0}].{1} WHERE {2} = @ID", propertyTableEntity.Schema, propertyTableEntity.Name,
        ...
        public void InsertDataUsingSqlBulkCopy<T>(IEnumerable<T> data, string schema = "")
        {
            try
            {
                ...
                if (propertyTableEntity != null)
                {
                    ...
                    var table = string.Format("{0}.{1}", propertyTableEntity.Schema, propertyTableEntity.Name);
                    using (IDbConnection cn = Connection)
                    {
        ...
        public virtual List<T> GetPage<T>(object predicate = null, IList<ISort> sort = null, int page = 0, int resultsPerPage = 10) where T : class
        {
            ...
            using (var cn = Connection)
            {
                ...
                var list = cn.GetPage<T>(predicate, sort, page, resultsPerPage).AsEnumerable();
                if (list.Any())
                {
                    data = list.ToList();
                }
        ...
        public TableAttribute GetTableFromType(Type type)
        {
            // Get instance of the attribute.
            var attributeAdo = (TableAttribute)Attribute.GetCustomAttribute(type, typeof(TableAttribute));
            if (attributeAdo == null) throw new ArgumentException("The attribute Table was not found", "type");
            return attributeAdo;
        }
